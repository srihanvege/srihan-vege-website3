...
const Section = ({ id, title, children }) => (
  <section id={id} className="scroll-mt-24 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
    <motion.h2 initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4 }} className="text-2xl sm:text-3xl font-semibold tracking-tight mb-6 text-slate-900 dark:text-white">{title}</motion.h2>
    {children}
  </section>
);

const LinkIcon = ({ href, children, title }) => (
  <a href={href} target="_blank" rel="noreferrer" title={title} className="inline-flex items-center gap-2 text-sm text-sky-700 hover:underline dark:text-sky-400">{children}</a>
);
    ...
    <div className="min-h-screen bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <header className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/70 dark:supports-[backdrop-filter]:bg-slate-900/70 bg-white/80 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-800">
        <nav className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-6 text-sm">
            ...
            <Button asChild className="rounded-md"><a href={INFO.resumeUrl} target="_blank" rel="noreferrer"><Download className="w-4 h-4 mr-2" /> Resume</a></Button>
            <ThemeToggle />
          </div>
        </nav>
      </header>
      ...
      <section id="home" className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid md:grid-cols-[1fr,220px] gap-10 items-center">
          <div>
            <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-4xl sm:text-5xl font-extrabold tracking-tight">{INFO.name}</motion.h1>
          ...
          {PROJECTS.map((p, idx) => (
            <Card key={idx} className="rounded-xl hover:shadow-sm transition-shadow border-slate-200 dark:border-slate-800">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">{p.tags.map((t) => <span key={t} className="text-xs px-2 py-1 rounded-full bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-100">{t}</span>)}</div>
      ...
      <Section id="skills" title="Interests">
        <Card className="rounded-xl border-slate-200 dark:border-slate-800">
          <CardContent className="p-6">
            <div className="flex flex-wrap gap-2">{SKILLS.map((s) => <span key={s} className="px-3 py-1 rounded-full bg-slate-100 text-slate-800 text-sm dark:bg-slate-800 dark:text-slate-100">{s}</span>)}</div>
